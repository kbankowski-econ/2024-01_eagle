...
dynare('shock_germany_gy.mod', sprintf('-I%s/%s/submodules', project_path, 'eagleParsingTemp_sim_BIG1'), 'savemacro', 'json=compute');
...
%%
serToPlot = (eabGyDatabank.EAB_pic4-eabGyDatabank.EAB_pic4(qq(0,4)))*100;
plot(serToPlot{qq(1,1): qq(50,4)});
title('EAB pic4')
ylabel('p.p. deviation from steady state')

close all;
serToPlot = (eabGyDatabank.EAB_y/eabGyDatabank.EAB_y(qq(0,4))-1)*100;
plot(serToPlot{qq(1,1): qq(50,4)});
title('EAB\_y')
ylabel('xxx')

close all;
serToPlot = (eabGyDatabank.EAA_y/eabGyDatabank.EAA_y(qq(0,4))-1)*100;
plot(serToPlot{qq(1,1): qq(50,4)});
title('EAA\_y')
ylabel('xxx')

close all;
serToPlot = (eabGyDatabank.EA_y/eabGyDatabank.EA_y(qq(0,4))-1)*100;
plot(serToPlot{qq(1,1): qq(50,4)});
title('EA\_y')
ylabel('xxx')

close all;
serToPlot = (eabGyDatabank.EAA_pi/eabGyDatabank.EAA_pi(qq(0,4))-1)*100;
plot(serToPlot{qq(1,1): qq(50,4)});
ylabel('xxx')

close all;
serToPlot = (eabGyDatabank.EAB_pnt/eabGyDatabank.EAB_pnt(qq(0,4))-1)*100;
plot(serToPlot{qq(1,1): qq(50,4)});
ylabel('xxx')

eabGyDatabank.EAB_pnt
pct(eabGyDatabank.EAB_pi)

close all;
serToPlot = (eabGyDatabank.EAB_cg/eabGyDatabank.EAB_cg(qq(0,4))-1)*100;
plot(serToPlot{qq(1,1): qq(50,4)});
title('EAB\_cg')
ylabel('xxx')
...
close all;
serToPlot = (eabGyDatabank.EAB_cgy-eabGyDatabank.EAB_cgy(qq(0,4)))*100;
plot(serToPlot{qq(1,1): qq(50,4)});
title('EAB_cgy')
ylabel('xxx')
...
close all;
serToPlot = (eabGyDatabank.EAA_pic-eabGyDatabank.EAA_pic(qq(0,4)))*100;
plot(serToPlot{qq(1,1): qq(50,4)});
title('EAA\_pic')
ylabel('xxx')

close all;
serToPlot = (eabGyDatabank.EAB_pint-eabGyDatabank.EAB_pint(qq(0,4)))*100;
plot(serToPlot{qq(1,1): qq(50,4)});
ylabel('xxx')

close all;
serToPlot = (eabGyDatabank.EAA_pint-eabGyDatabank.EAA_pint(qq(0,4)))*100;
plot(serToPlot{qq(1,1): qq(50,4)});
ylabel('xxx')
close all;
serToPlot = (eabGyDatabank.EAA_piim-eabGyDatabank.EAA_piim(qq(0,4)))*100;
plot(serToPlot{qq(1,1): qq(50,4)});
ylabel('xxx')
...
DeFiscalSimOutput = load(fullfile(project_path, 'eagleParsingTemp_sim_BIG1', 'modFiles', 'shock_germany_gy', 'Output', 'shock_germany_gy_results.mat'));
M_ = DeFiscalSimOutput.M_;

dataRange = qq(0, 4): qq(0, 4)+size(DeFiscalSimOutput.oo_.endo_simul', 1) - 1;
endoStruct = struct(); ssStruct = struct(); irfStruct = struct();

endoStruct = databank.fromArray( ...
    DeFiscalSimOutput.oo_.endo_simul' ...
    , M_.endo_names ...
    , dataRange(1) ...
);
ssStruct = databank.fromArray( ...
    repmat(DeFiscalSimOutput.oo_.steady_state', numel(dataRange), 1) ...
    , M_.endo_names ...
    , dataRange(1) ...
);
for aParam = string(reshape(DeFiscalSimOutput.M_.param_names, 1, []))
...
function panelContributions(contributionSeries, projectPath, subProjectPath)
    ...
    % Save graph
    fileName = fullfile(projectPath, "docs/GermanyfiscalContributions_Big1");
