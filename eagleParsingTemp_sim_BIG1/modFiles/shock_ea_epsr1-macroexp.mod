...
-RWEAB_numc
-RWUS_numc
)*((EAA_pex*RWEAA_rer)/(RWEAA_gammaimcdag*RW_pimc))^(-RW_mumc)*RW_imc/(1-RWEAA_gammaimc);
// Price of the consumption good (import)
RW_pimc^(1-RW_mumc) =
...
-RWEAB_numc
-RWUS_numc
)*((EAA_pex*RWEAA_rer)/RWEAA_gammaimcdag)^(1-RW_mumc);
// Private consumption good (import) inflation
...
-RWEAB_numi
-RWUS_numi
)*((EAA_pex*RWEAA_rer)/(RWEAA_gammaimidag*RW_pimi))^(-RW_mumi)*RW_imi/(1-RWEAA_gammaimi);
// Price of the consumption good (import)
RW_pimi^(1-RW_mumi) =
...
-RWEAB_numi
-RWUS_numi
)*((EAA_pex*RWEAA_rer)/RWEAA_gammaimidag)^(1-RW_mumi);
// Private consumption good (import) inflation
...
-RWEAB_numcg
-RWUS_numcg
)^(1/RW_mumcg)*((1-RWEAA_gammaimcg)*RWEAA_imcg)^(1-1/RW_mumcg);
// Demand for bilateral consumption import goods
RWEAB_imcg = RWEAB_numcg*((EAB_pex*RWEAB_rer)/(RWEAB_gammaimcgdag*RW_pimcg))^(-RW_mumcg)*RW_imcg/(1-RWEAB_gammaimcg);
RWEAA_imcg = (1
-RWEAB_numcg
-RWUS_numcg
)*((EAA_pex*RWEAA_rer)/(RWEAA_gammaimcgdag*RW_pimcg))^(-RW_mumcg)*RW_imcg/(1-RWEAA_gammaimcg);
// Price of the consumption good (import)
RW_pimcg^(1-RW_mumcg) =
...
-RWEAB_numcg
-RWUS_numcg
)*((EAA_pex*RWEAA_rer)/RWEAA_gammaimcgdag)^(1-RW_mumcg);
// Private consumption good (import) inflation
...
-RWEAB_numig
-RWUS_numig
)^(1/RW_mumig)*((1-RWEAA_gammaimig)*RWEAA_imig)^(1-1/RW_mumig);
// Demand for bilateral consumption import goods
RWEAB_imig = RWEAB_numig*((EAB_pex*RWEAB_rer)/(RWEAB_gammaimigdag*RW_pimig))^(-RW_mumig)*RW_imig/(1-RWEAB_gammaimig);
RWEAA_imig = (1
-RWEAB_numig
-RWUS_numig
)*((EAA_pex*RWEAA_rer)/(RWEAA_gammaimigdag*RW_pimig))^(-RW_mumig)*RW_imig/(1-RWEAA_gammaimig);
// Price of the consumption good (import)
RW_pimig^(1-RW_mumig) =
...
-RWEAB_numig
-RWUS_numig
)*((EAA_pex*RWEAA_rer)/RWEAA_gammaimigdag)^(1-RW_mumig);
// Private consumption good (import) inflation
...
-USEAA_numc
-USRW_numc
)*((EAB_pex*USEAB_rer)/(USEAB_gammaimcdag*US_pimc))^(-US_mumc)*US_imc/(1-USEAB_gammaimc);
// Price of the consumption good (import)
US_pimc^(1-US_mumc) =
...
-USEAA_numc
-USRW_numc
)*((EAB_pex*USEAB_rer)/USEAB_gammaimcdag)^(1-US_mumc);
// Private consumption good (import) inflation
...
-USEAA_numi
-USRW_numi
)*((EAB_pex*USEAB_rer)/(USEAB_gammaimidag*US_pimi))^(-US_mumi)*US_imi/(1-USEAB_gammaimi);
// Price of the consumption good (import)
US_pimi^(1-US_mumi) =
...
-USEAA_numi
-USRW_numi
)*((EAB_pex*USEAB_rer)/USEAB_gammaimidag)^(1-US_mumi);
// Private consumption good (import) inflation
...
-USEAA_numcg
-USRW_numcg
)^(1/US_mumcg)*((1-USEAB_gammaimcg)*USEAB_imcg)^(1-1/US_mumcg);
// Demand for bilateral consumption import goods
USRW_imcg = USRW_numcg*((RW_pex*USRW_rer)/(USRW_gammaimcgdag*US_pimcg))^(-US_mumcg)*US_imcg/(1-USRW_gammaimcg);
USEAB_imcg = (1
-USEAA_numcg
-USRW_numcg
)*((EAB_pex*USEAB_rer)/(USEAB_gammaimcgdag*US_pimcg))^(-US_mumcg)*US_imcg/(1-USEAB_gammaimcg);
// Price of the consumption good (import)
US_pimcg^(1-US_mumcg) =
...
-USEAA_numcg
-USRW_numcg
)*((EAB_pex*USEAB_rer)/USEAB_gammaimcgdag)^(1-US_mumcg);
// Private consumption good (import) inflation
...
-USEAA_numig
-USRW_numig
)^(1/US_mumig)*((1-USEAB_gammaimig)*USEAB_imig)^(1-1/US_mumig);
// Demand for bilateral consumption import goods
USRW_imig = USRW_numig*((RW_pex*USRW_rer)/(USRW_gammaimigdag*US_pimig))^(-US_mumig)*US_imig/(1-USRW_gammaimig);
USEAB_imig = (1
-USEAA_numig
-USRW_numig
)*((EAB_pex*USEAB_rer)/(USEAB_gammaimigdag*US_pimig))^(-US_mumig)*US_imig/(1-USEAB_gammaimig);
// Price of the consumption good (import)
US_pimig^(1-US_mumig) =
...
-USEAA_numig
-USRW_numig
)*((EAB_pex*USEAB_rer)/USEAB_gammaimigdag)^(1-US_mumig);
// Private consumption good (import) inflation
...
EAA_phirr_=0; //if we don't want persistent shock
EAB_phirr_=0; //if we don't want persistent shock
