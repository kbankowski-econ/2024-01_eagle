...
% Generate MATLAB-based country decomposition plots (for NGEU we want a bit of a different chart)
if modelName == "shock_ngeu"
    plotting.WP.eaCntryDecomposition(contributionSeries, modelName ...
        , "redateNewDate", qq(2021, 1) ...
        , "plottingRange", yy(2021):yy(2030) ...
    );
    % Spillover panels (figure effectNGEUspillovers in the paper)
    plotting.WP.ngeuSpillovers(simProcResults.irfValues, modelName);
else
    plotting.WP.eaCntryDecomposition(contributionSeries, modelName ...
    );    
end
