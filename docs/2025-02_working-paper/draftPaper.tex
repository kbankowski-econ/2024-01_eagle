...
\end{equation}
Figure~\ref{finalGoodProduction} offers a detailed schematic representation of the production of a final good of type $s$.\footnote{The CES aggregation framework also implies that the prices of final goods are composite indices of the prices of their underlying components. Specifically, the price of a final good of type $s$, denoted $P_{t}^s$, the price of the tradable component $P_{TT,t}^s$, and the price of imported tradable goods $P_{IM,t}^s$ are defined as follows:
\[
P_{t}^s = \left[\nu_s\left(P_{TT,t}^s\right)^{1-\mu_s} + \left(1-\nu_s\right)\left(P_{NT,t}\right)^{1-\mu_s}\right]^{\frac{1}{1-\mu_s}},
\]
\[
P_{TT,t}^s = \left[\nu_{Ts}\left(P_{HT,t}\right)^{1-\mu_{Ts}} + \left(1-\nu_{Ts}\right)\left(P_{IM,t}^s\right)^{1-\mu_{Ts}}\right]^{\frac{1}{1-\mu_{Ts}}},
\]
\[
P_{IM,t}^s = \left[\sum_{CO \neq H} \nu_{Ms}^{H,CO}\left(P_{IM,t}^{s,CO}\right)^{1-\mu_{Ms}}\right]^{\frac{1}{1-\mu_{Ms}}}.
\]
Here, $\nu_s$, $\nu_{Ts}$, and $\nu_{Ms}^{H,CO}$ are the share parameters governing the aggregation at each respective level, and $\mu_s$, $\mu_{Ts}$, and $\mu_{Ms}$ denote the elasticities of substitution. The price $P_{NT,t}$ corresponds to domestically produced non-tradable goods, $P_{HT,t}$ denotes the price of home-produced tradable goods, and $P_{IM,t}^{s,CO}$ refers to the price of imports of type $s$ from country or region $CO$. The structure of price aggregation mirrors the quantity aggregation.}
