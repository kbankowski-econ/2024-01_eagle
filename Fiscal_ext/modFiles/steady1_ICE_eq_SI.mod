...
@#include "params_ICE_eq_SI.mod"

@#include "initval_ICE_eq_SI.mod"
...
RW_nux,  .65;
US_nux,  .85;
%EAB_nux, 0.25; %Note: calibrated in steady2 
end;
steady(homotopy_steps = 5);
...
//fixed cost in T production is calibarted in steady2
%disp('psit')
%homotopy_setup;
%EAA_psit, 0.3;
%RW_psit,  0.2;
...
steady(homotopy_steps = 100, solve_algo = 6);
...
save_params_and_steady_state('eagle_steady_stage1_ICE_eq_SI.txt');
