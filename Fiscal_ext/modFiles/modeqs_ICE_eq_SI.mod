...
@# for it in countries - [ co ]
 +@{it}@{co}_pim*@{co}@{it}_rer*(@{it}@{co}_imc+@{it}@{co}_imi+@{it}@{co}_imx)*@{it}_size/@{co}_size
@# endfor
...
// Total imports 
@{co}_im = @{co}_imc+@{co}_imi+@{co}_imx;

// Total import deflator
@{co}_im*@{co}_pim  = @{co}_pimc*@{co}_imc+@{co}_pimi*@{co}_imi+@{co}_pimx*@{co}_imx;
...
@{co}_yst = @{co}_sh*(@{co}_htc+@{co}_hti)
@# for it in countries - [ co ]
...
@# for it in countries - [ co ]
@{co}@{it}_im = @{co}@{it}_imi + @{co}@{it}_imc + @{co}@{it}_imx;
...
//Price markup in tradable and non-tradable sectors
@{co}_thetan = (1-@{co}_rhothetan)*@{co}_thetanbar+@{co}_rhothetan*@{co}_thetan(-1)+@{co}_epsthetan;
@{co}_thetat = (1-@{co}_rhothetat)*@{co}_thetatbar+@{co}_rhothetat*@{co}_thetat(-1)+@{co}_epsthetat;
...
@{co}_imy = (@{co}_pimc*@{co}_imc+@{co}_pimi*@{co}_imi+@{co}_pimx*@{co}_imx)/(@{co}_py*@{co}_y);
...
// International transaction cost ('risk premium')
@#if !steady
@{co}_gammabh = @{co2}_gammab1*(exp(@{co}@{co2}_rer*@{co}_bh/@{co2}_pic/(@{co}_py*@{co}_y)-@{co}_bfytarget)-1); //pj-@{co2}_rp;
@#else
@{co}_bh = (@{co}_bfytarget*@{co}_py*@{co}_y)/@{co}@{co2}_rer*@{co2}_pic;
@#endif
