...
### Formatting plan: align with the spending-model paper

Reference: `~/Developer/2025-05_spendingModelling/docs/2026-06_wp-imf/draftPaper.tex`
(IMF WP, August 2026). Same class, fonts, margins, footer, `ecta` + natbib,
`[H]` floats and one-sentence-per-line already match. Work through the items
in order; each is one commit.

1. **Preamble.** Replace the EAGLE preamble with the reference one and keep
   only the EAGLE extras that are used (`pdflscape` for the 17 landscape
   tables, `pgffor`, the country-code and note macros). Drops `lipsum`,
   `blindtext`, `multicol`. Line spacing 1.5 to 1.15. Links one dark blue
   (`linkcol`) instead of blue/red. `\captionsetup{labelfont=bf,font=small,labelsep=period}`.
2. **Section titles.** Centred small caps to left-aligned bold with the
   number in `linkcol`, plus the reference `\titlespacing`. Section names in
   Title Case ("Introduction and Motivation"), as in the reference.
3. **Note macros.** `\source`/`\desc` from `\small` to `\footnotesize`,
   "Notes:" to "Note:"; add `\fignote{sources}{note}` and
   `\tabnote{sources}{note}` and use them on every figure and table, which
   also fills the 5 empty `\source{Own calculations based on ....}`.
4. **Title page.** Blue rule under the title, author footnote with
   affiliation, e-mail and disclaimer in one sentence, close `abstract`
   before Keywords/JEL (the `\clearpage` now sits inside it), then
   `\clearpage`. Drop the "Preliminary Draft" banner at submission.
5. **Front matter.** Remove the table of contents, list of figures and list
   of tables; the reference has none.
6. **Figure sizing.** Reference figures are produced at their final size
   (425 x 532 pt, about 0.93 of the text width) and included without
   `width=`. EAGLE scales 452 pt squares to `1.1\textwidth` inside
   `\makebox`, and the contribution charts are 184 x 47 pt scaled up
   sevenfold. Set the export sizes in `plotIRFs.py`, the contribution and
   NGEU scripts and `plotting.WP.*` to about 425 pt wide with one font
   size, then include at natural size.
7. **Vector figures.** Replace the four PNGs (`importMagnitude`,
   `importContent` from MATLAB `print -dpng`; `OECD_table`,
   `effectNGEUspillovers`) with PDFs generated by a script.
8. **Captions.** Title Case for every figure and table caption
   ("Euro Area Monetary Policy Shock: IRFs"); no trailing period; colon
   instead of "--" for the IRF/decomposition pairs.
9. **Tables.** Generated files contain only the `tabular` with a
   "Generated by ... Do not edit by hand" header; `\caption`, `\label`,
   `\footnotesize` and `\arraystretch{1.2}` live in the paper, as in the
   reference. Regenerate all 22 files from their `.m` scripts.
10. **Numbering.** Figures and tables numbered continuously through the
    appendices (reference: Figure 16 in Appendix), instead of resetting to
    A.1/B.1. Keep lettered appendix sections and equations per section.
11. **References in text.** `\citet`/`\citep` only (3 `\citealp` left);
    `Figure~\ref` and `Table~\ref` with a tie everywhere.
12. **Prose conventions.** `---` instead of the Unicode em dash; "percent"
    in prose instead of `\%`; `%====` banner comments before each section.
13. **Bibliography.** Copy the cited entries into a local `references.bib`
    next to the paper so the build no longer depends on the machine-local
    `\bibliopath`.
